...
if($_SERVER['REQUEST_METHOD'] == "POST"){
    ...
    if(!$flag) {
    	...
    	if(empty($existing_data)) {
    		$arr1[] = array("fname" => $firstName, "lname" => $lastName, "gender" => $gender_, "birthday" => $dob,"religion" => $religion,"presentaddress" => $presentAdd, "permanentaddress" => $permAdd, "phone" => $phoneNo, "email" => $email, "website" => $webLink, "uname" => $userName, "password" => $_pass);
    		$result = write(json_encode($arr1));		
    	}
    	else{
    		$existing_data_decode = json_decode($existing_data);
    		array_push($existing_data_decode, array("fname" => $firstName, "lname" => $lastName, "gender" => $gender_, "birthday" => $dob,"religion" => $religion,"presentaddress" => $presentAdd, "permanentaddress" => $permAdd, "phone" => $phoneNo, "email" => $email, "website" => $webLink, "uname" => $userName, "password" => $_pass));
    		write("");
